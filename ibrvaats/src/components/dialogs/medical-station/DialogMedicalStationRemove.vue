  ...
  <Dialog v-model="dialog" :width="640">
    <Card>
      <v-card-title class="bg-primary pa-4">
        <Label header> Remove Medical Station</Label>
      </v-card-title>
      <v-card-text>
...
import { remove } from "@/api/medical-stations";
...
const medicalStation = computed(syncProp(propRef, emit, "medicalStation"));

const submitHandler = async () => {
  try {
    isLoading.value = true;
    const result = await remove(medicalStation.value);
