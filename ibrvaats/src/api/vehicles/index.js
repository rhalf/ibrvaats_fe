...
export const getByPlateNumber = async (plateNumber) => {
  const q = await query(collectionRef, where("plateNumber", "==", plateNumber));
  const snapshots = await getDocs(q);
  return toArray(snapshots);
};
...
//owner
export const searchByPolice = async ({
  searchText,
  columnName,
  orderDirection,
  limitNumber,
...
export const nextByPolice = async ({
  columnName,
  orderDirection,
  limitNumber,
}) => {
...
export const prevByPolice = async ({
  columnName,
  orderDirection,
  limitNumber,
}) => {
...
export const getAllByPolice = async () => {
  const { uid } = await getCurrentUser();
...
export const removePolice = async (item) => {
  item.updatedAt = Timestamp.fromDate(new Date());
  item.owner = null;
  const documentRef = doc(firestore, collectionName, item.id);
  return await setDoc(documentRef, item);
...
export const countByPolice = async () => {
  const { uid } = await getCurrentUser();
...
//Police
export const getByPoliceAndId = async (id) => {
  const { uid } = await getCurrentUser();
